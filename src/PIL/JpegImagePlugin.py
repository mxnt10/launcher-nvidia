...
def APP(self, marker):
    ...
    if marker == 0xFFE0 and s[:4] == b"JFIF":
        ...
        if "exif" not in self.info:
            ...
            self.info["exif"] = s  # FIX: value will change
    elif marker == 0xFFE2 and s[:5] == b"FPXR\0":
        # extract FlashPix information (incomplete)
        self.info["flashpix"] = s  # FIX: value will change
...
def DQT(self):
    ...
    # FIX: The quantization tables can be used to estimate the
    # compression quality.
...
def _save(im, fp):
    ...
    def validate_qtables(q_tables):
        if q_tables is None:
            return q_tables
        if isinstance(q_tables, str):
            try:
                lines = [
                    int(num)
                    for line in q_tables.splitlines()
                    for num in line.split("#", 1)[0].split()
                ]
            except ValueError as e:
                raise ValueError("Invalid quantization table") from e
            else:
                q_tables = [lines[s: s + 64] for s in range(0, len(lines), 64)]
        if isinstance(q_tables, (tuple, list, dict)):
            if isinstance(q_tables, dict):
                q_tables = [
                    q_tables[key] for key in range(len(q_tables)) if key in q_tables
                ]
            elif isinstance(q_tables, tuple):
                q_tables = list(q_tables)
            if not (0 < len(q_tables) < 5):
    ...
    # says "progression"
    # FIX: issue a warning if the wrong form is used (post-1.1.7)
...
def _save_cjpeg(im, fp, filename):
    # ALTERNATIVE: handle JPEGs via the IJG command line utilities.
    temp_file = im._dump()
    subprocess.check_call(["cjpeg", "-outfile", filename, temp_file])
    try:
        os.unlink(temp_file)
    except OSError:
        pass
